...
class _AccountDetailPageState extends ConsumerState<AccountDetailPage> {
  ...
  Future<void> _calibrateBalance() async {
    final repo = ref.read(repositoryProvider);
    final current = await repo.getAccountBalance(widget.account.id);
    if (!mounted) return;
    ...
    final target = await showDialog<double>(
      ...
      builder: (dialogContext) => BeeAlertDialog(
        ...
        content: Column(mainAxisSize: MainAxisSize.min, children: [
          Text(
              '当前账面余额：${current.toStringAsFixed(2)} ${widget.account.currency}'),
          const SizedBox(height: 12),
          TextField(
            controller: input,
            keyboardType: const TextInputType.numberWithOptions(
                decimal: true, signed: true),
            decoration: const InputDecoration(labelText: '实际余额（信用卡欠款填负数）'),
          ),
        ]),
        actions: [
          TextButton(
          ...
          FilledButton(
              onPressed: () => Navigator.pop(
                  dialogContext, double.tryParse(input.text.trim())),
              child: const Text('确认')),
        ],
      ),
    );
    ...
    if (target == null ||
        !target.isFinite ||
        (target - current).abs() < 0.005) {
      return;
    }
    await repo.createAdjustmentTransaction(
      ledgerId: widget.account.ledgerId,
    ...
    await PostProcessor.run(ref, ledgerId: widget.account.ledgerId);
  }
