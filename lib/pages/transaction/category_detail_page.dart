    ...
    final summaryAsync = filteredTransactionsAsync.when(
      ...
      data: (transactions) {
        ...
        final totalAmount = transactions.fold(
          0.0,
          (sum, t) => sum + (t.nativeAmount ?? t.amount),
        );
        ...
        itemBuilder: (context, index) {
          ...
          if (item.isHeader) {
            ...
            return TransactionListItem(
              ...
              onTap: () async {
                final categoryData = ref.read(
                  _categoryStreamProvider(widget.categoryId),
                );
...
// 基础数据流：监听分类信息变化
final _categoryStreamProvider = StreamProvider.family<db.Category?, int>((
  ref,
  categoryId,
) {
...
// 排序状态管理
final _categorySortTypeProvider = StateProvider.family<SortType, int>((
  ref,
  categoryId,
) {
      ...
      child: GestureDetector(
        ...
        child: Container(
          padding: EdgeInsets.symmetric(
            horizontal: LiquidTheme.isActive(context) ? 14 : 12,
            vertical: LiquidTheme.isActive(context) ? 12 : 6,
          ),
