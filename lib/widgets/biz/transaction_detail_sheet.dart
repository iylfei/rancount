              ...
              child: ListView(
                ...
                children: [
                  if (_nonBlank(transaction.itemDescription) != null)
                    _detailRow(context, l10n.transactionDetailProduct,
                        transaction.itemDescription!.trim()),
                  if (_nonBlank(categoryName) != null)
                    _detailRow(context, l10n.transactionDetailCategory,
                        categoryName!.trim()),
                  if (_nonBlank(transaction.merchant) != null)
                    _detailRow(context, l10n.transactionDetailMerchant,
                        transaction.merchant!.trim()),
                  if (_nonBlank(transaction.paymentChannel) != null)
                    _detailRow(context, l10n.transactionDetailPaymentChannel,
                        transaction.paymentChannel!.trim()),
                  if (_nonBlank(accountDisplay) != null)
                    _detailRow(context, l10n.transactionDetailAccount,
                        accountDisplay!.trim()),
                  _detailRow(
                    context,
                  ...
                  if (_nonBlank(transaction.note) != null)
                    _detailRow(context, l10n.transactionDetailNote,
                        transaction.note!.trim()),
                  if (tags.isNotEmpty)
                    _detailRow(
                        context, l10n.transactionDetailTags, tags.join(' · ')),
                  if (attachmentCount > 0)
                    _detailRow(context, l10n.transactionDetailAttachments,
                        '$attachmentCount'),
                  if (flags.isNotEmpty)
                    _detailRow(context, l10n.transactionDetailFlags,
                        flags.join(' · ')),
                ],
              ),
            ),
